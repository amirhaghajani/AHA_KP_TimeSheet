...
function kwndSaveWHs_OnClose() {
    $("#kwndSaveWorkHours").data("kendoWindow").close()
}
...
function Refresh_GrdEditWorkHour() {
    ...
    $.ajax({
        ...
        success: function (response) {
            _WorkHours = response;
            $("#GrdEditWorkHour").data("kendoGrid").dataSource.read();
        },
        error: function (e) {
        }
    });
...
function ResetSaveWindow() {
    $("#ddlProjects").data("kendoDropDownList").select(0)
    $("#ddlTasks").data("kendoDropDownList").select(0)
    $("#txtDescription").val(""); 
    $("#TaskPanel").hide();
}